function [qT, qT_d, qT_dd, tSolving] = solveSDPProblem(robot, tasksTable, q0, q0_d, ...
            ...
            jointsLBound, jointsUBound, obstacle, simLength, dt, dampFactor)
    qT = [];
    qT_d = [];
    qT_dd = [];
    ...
    for i = 1:simLength
        ...
        while currPriority <= maxPriority
            ...
            [A, b, C, d, C_daux, d_daux, eqDim, ineqDim] = SDPLoadTasksInMatrices(currPriority, tasksTable, qC_dd);
            ...
            % Adding a regularization factor at the last optimization
            if currPriority == maxPriority
                A_comp = [A_comp; dampFactor*eye(robot.n)];
                b_comp = [b_comp; zeros(robot.n,1)];
                bStruct(1) = bStruct(1) + robot.n;
            end          
